...
proc execute_request {identity jmsg} {
    variable ph
    variable exec_counter
    incr exec_counter
    set ph [dict get $jmsg header]

    iopub [jmsg::status $ph busy]
    ...
    shell $identity $jmsg
    iopub [jmsg::status $ph idle]
}

proc shell {identity jmsg} {
    variable ::master
    thread::send -async $master [list to_shell $identity  $jmsg]
}

proc iopub {jmsg} {
    variable ::master
    thread::send -async $master [list to_iopub $jmsg]
}