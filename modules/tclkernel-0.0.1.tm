...
proc recv_shell {frames} {
     ...
     handle_$msgtype $jmsg
}


proc recv_control {alias frames} {
    # shell control an available iopub channel to
     # show status
     if {[info commands ::tmq::socket::iopub] eq {}} {
        puts "No IOPUB connection yet, waiting.."
        after 100 [list recv_control $alias $frames]
     }
     set jmsg [jmsg::new $frames]
...
proc recv_iopub {alias zmsg} {
     puts "iopub: << [tmq::display $zmsg]"
}

proc handle_kernel_info_request {jmsg} {
