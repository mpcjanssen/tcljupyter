...
# All Tcl 8.7+ interps can [load] thread 3.0b4
#
...
package ifneeded [string tolower thread] 3.0b4 \
    [list load [file join $dir libtcl9thread3.0b4.so] [string totitle thread]]
package ifneeded [string totitle thread] 3.0b4 \
    [list package require -exact [string tolower thread] 3.0b4]

# package ttrace uses some support machinery.

# In Tcl 8.7+ interps; use [::apply]

package ifneeded ttrace 3.0b4 [list ::apply {{dir} {
    ...
    } elseif {![catch {zipfs mount //zipfs:/lib/thread [file join $dir libtcl9thread3.0b4.so]}]} {
...
package ifneeded Ttrace 3.0b4 \
    [list package require -exact ttrace 3.0b4]
